...
export async function fetchSheetsData(range = "2025!A6:AK") {
  ...
  return rows
    ...
    .map((row) => {
      ...
      return {
        ...
        cost: parseRupiah(row[headerIndex["Cost"]] || row[9] || "0"),
        reach: Number.parseInt(row[headerIndex["Reach"]] || row[10] || "0"),
        impressions: Number.parseInt(
          row[headerIndex["Impressions"]] || row[11] || "0"
        ),
        cpm: parseRupiah(row[headerIndex["CPM"]] || row[12] || "0"),
        click: Number.parseInt(row[headerIndex["Link Clicks"]] || row[13] || "0"),
        cpc: parseRupiah(row[headerIndex["CPC"]] || row[14] || "0"),
        wamasuk,
        cpr: parseRupiah(row[headerIndex["CPR"]] || row[16] || "0"),
        ...
        omz: parseRupiah(row[headerIndex["Omzet"]] || row[36] || "0"),
      }
    })
}
